    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-[#0096FF]/10 to-white text-black font-sans relative">
      ...
      {!isTestReady ? (
        <main className="max-w-3xl mx-auto pt-32 px-10 pb-20 text-center">
          ...
          <p className="text-gray-600 text-lg mb-12">Please test your microphone before proceeding.</p>
          <div className="bg-white p-12 rounded-[2rem] shadow-xl border border-gray-100 flex flex-col items-center">
            <p className="text-xl font-medium text-gray-700 mb-8">
              {micStatus === 'idle' ? 'Click and say "Hello"' : micStatus === 'testing' ? 'Listening...' : 'Audio detected!'}
            </p>
            <button onClick={handleMicTest} disabled={micStatus === 'success'} className={`w-28 h-28 rounded-full flex items-center justify-center shadow-lg transform transition-all ${micStatus === 'testing' ? 'bg-red-500 hover:bg-red-600 scale-110' : micStatus === 'success' ? 'bg-green-500' : 'bg-black hover:bg-gray-800'}`}>
            ...
            {micStatus === 'success' && (
              <button onClick={startActualTest} className="mt-10 bg-[#0096FF] text-white font-bold py-4 px-12 rounded-full shadow-lg hover:bg-blue-600 transition-all transform hover:-translate-y-1">Proceed to Evaluation</button>
            )}
          </div>
        </main>
      ) : (
        <main className="max-w-4xl mx-auto pt-20 px-10 pb-20">
          ...
          <div className="flex flex-col items-center justify-center">
            <button onClick={toggleRecording} className={`w-24 h-24 rounded-full flex items-center justify-center shadow-lg transform transition-all ${isRecording ? 'bg-red-500 animate-pulse' : 'bg-black hover:bg-gray-800'}`}>
            ...
            <p className={`mt-6 font-bold text-lg ${isRecording ? 'text-red-500' : 'text-gray-500'}`}>{isRecording ? 'Recording... Click to stop.' : (hasRecorded ? 'Recording saved!' : 'Click to start recording')}</p>
            {hasRecorded && (
