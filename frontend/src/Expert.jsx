...
export default function Expert() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-[#005FA3]/5 to-white text-black font-sans relative">
        ...
        <main className="max-w-3xl mx-auto pt-32 px-10 pb-20 text-center">
          ...
          <div className="bg-white p-12 rounded-[2rem] shadow-xl border border-gray-100 flex flex-col items-center">
            <p className="text-xl font-medium text-gray-700 mb-8">
              {micStatus === 'idle' ? 'Click the mic to begin test' : micStatus === 'testing' ? 'Listening for input...' : 'Microphone is optimized!'}
            </p>
            <button 
              ...
              className={`w-28 h-28 rounded-full flex items-center justify-center shadow-lg transform transition-all ${
                micStatus === 'testing' ? 'bg-red-600 animate-pulse scale-110' : 
                micStatus === 'success' ? 'bg-green-600' : 'bg-[#005FA3] hover:bg-[#004A80] hover:scale-105'
              }`}
            >
            ...
            {micStatus === 'success' && (
              <button onClick={startActualTest} className="mt-10 bg-[#005FA3] text-white font-bold py-4 px-12 rounded-full shadow-lg hover:bg-[#004A80] transition-all transform hover:-translate-y-1">
        ...
        <main className="max-w-4xl mx-auto pt-20 px-10 pb-20">
          ...
          <div className="bg-white p-10 rounded-[2rem] shadow-xl border border-gray-100 mb-10">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-[#005FA3]">Reading Material</h2>
              <span className="text-sm font-bold text-gray-400 bg-gray-100 px-3 py-1 rounded-full">{currentIndex + 1} / 25</span>
            </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <p className={`mt-6 font-bold text-lg ${isRecording ? 'text-red-600' : 'text-gray-500'}`}>{isRecording ? 'Recording Expert Audio...' : hasRecorded ? 'Recording Secure' : 'Click to begin'}</p>
            {hasRecorded && (
              <button onClick={nextPassage} className="mt-8 bg-[#005FA3] text-white font-bold py-4 px-10 rounded-full shadow-lg hover:bg-[#004A80] transition-all">
                {currentIndex < 24 ? 'Next Passage \u2192' : 'Complete Evaluation \u2192'}
              </button>
            )}
          </div>
        </main>
      ...
      {/* Custom Modal aligned to your UI design */}
      {showConfirmModal && (
