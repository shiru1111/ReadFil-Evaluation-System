    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="text-center">
          <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight sm:text-5xl">Algorithm Simulation</h1>
          <p className="mt-4 text-xl text-gray-500">Trace the exact Needleman-Wunsch & MLD scoring path</p>
        </div>
          ...
          {passages.map((passage, index) => (
            <div key={passage.id} className="relative bg-gray-50 p-6 rounded-2xl border border-gray-200">
              <div className="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
                <h3 className="text-xl font-bold text-gray-800">Passage #{index + 1}</h3>
                {passages.length > 1 && (
                  <button 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <label className="block text-sm font-bold text-gray-700 uppercase tracking-wide">Target Text (Passage)</label>
                  <textarea 
                    rows={3}
                ...
                <div className="space-y-4">
                  <label className="block text-sm font-bold text-gray-700 uppercase tracking-wide">Spoken Text (Student)</label>
                  <textarea 
                    rows={3}
                    className="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 font-mono text-sm p-4 bg-white"
              ...
              <div className="mt-6 w-48 space-y-2">
                <label className="block text-sm font-bold text-gray-700 uppercase tracking-wide">Duration (Seconds)</label>
                <input 
                  type="number" 
                  step="0.1"
        ...
        {results && (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* OVERALL Score Cards */}
            <div>
              <h2 className="text-2xl font-bold mb-6 text-center">OVERALL CUMULATIVE SCORE</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 flex flex-col items-center justify-center text-center relative overflow-hidden group">
                   ...
                   <div className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-2">Total Accuracy</div>
                   <div className="text-5xl font-black text-blue-600">{results.overall_accuracy}%</div>
                   <div className="text-sm text-gray-500 mt-2">{results.total_correct_words} / {results.total_target_words} words</div>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 flex flex-col items-center justify-center text-center relative overflow-hidden group">
                   ...
                   <div className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-2">Total Speed (WCPM)</div>
                   <div className="text-5xl font-black text-purple-600">{results.overall_wcpm}</div>
                   <div className="text-sm text-gray-500 mt-2">Target: 150 WCPM</div>
                </div>
                ...
                <div className="bg-gradient-to-br from-black to-gray-800 p-6 rounded-2xl shadow-xl border border-gray-700 flex flex-col items-center justify-center text-center relative overflow-hidden group text-white">
                   ...
                   <div className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-2">Total Composite Score</div>
            ...
            <div className="space-y-8">
              <h2 className="text-2xl font-bold text-center border-b border-gray-300 pb-4">INDIVIDUAL TRANSCRIPT ANALYSIS</h2>
              {results.passages.map((passageResult, index) => (
                <div key={index} className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 p-8">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-2xl font-black text-gray-800">Passage #{index + 1}</h3>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b-2 border-gray-200">
                          ...
                          <th className="py-4 px-4 text-sm font-bold text-gray-500 uppercase tracking-wider">Status</th>
                          <th className="py-4 px-4 text-sm font-bold text-gray-500 uppercase tracking-wider">MLD Penalty</th>
